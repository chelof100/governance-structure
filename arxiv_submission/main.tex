...
\textbf{P3/4} & \textbf{Irreducible Governance Structure} (this paper) & \makecell[l]{10.5281/zenodo.19672597 (P3)\\10.5281/zenodo.19672608 (P4)} & TBD \\
...
admission-time snapshot $\mathcal{A}_{0,i}$. Let $P_{\mathrm{excess}}$ be the distribution of
actions drawn from states visited only due to excess allocation, with
$\mathrm{JS}(P_{\mathrm{excess}} \| P_{\mathcal{A}_{0,i}}) \geq \eta > 0$. If $a_i$ is
persistently over-served with bias $B_i(t) \geq \delta > 0$ over a window of $T$ steps,
then the IML deviation estimator satisfies
\[
  \widehat{D}(a_i,\,\mathcal{A}_{0,i}) \;\geq\; 0.40 \cdot f^2 \eta / 2 \;>\; 0,
\]
...
is at Jensen--Shannon distance $\geq f^2 \eta / 2$ from $P_{\mathcal{A}_{0,i}}$. Since $f > 0$
